...
Target and Observation Managers (TOMs) are meant to facilitate collaborative astronomical observing projects. A typical TOM allows its users to curate target lists, request observations of those targets at various observatories as well as manage and organize their data. [Read more :lucide-external-link:](https://tom-toolkit.readthedocs.io/en/stable/introduction/about.html){target="blank_"} about TOMs.

We developed a module for the TOM Toolkit, [tom_fink :lucide-external-link:](https://github.com/TOMToolkit/tom_fink){target="blank_"}, to easily connect Fink to any TOM instance.
...
As of `tom-fink` version 0.4, the module simply uses the [Fink REST API](api/getting_started.md) to retrieve alerts. Note that the Fink databases are updated **once a day** with the previous night alert data (hence you do not get live alerts for the moment). Users can perform:

- Search by LSST object ID
- Cone Search
- Search by derived alert tags
...
Credentials are provided when registering to Fink livestream (see [https://doc.lsst.fink-broker.org/en/latest/services/fink_client/ :lucide-external-link:](https://doc.lsst.fink-broker.org/en/latest/services/fink_client/){target="blank_"}). The available topic names and description can be found at [https://doc.lsst.fink-broker.org/en/latest/broker/filters/#available-topics :lucide-external-link:](https://doc.lsst.fink-broker.org/en/latest/broker/filters/#available-topics){target="blank_"}. Note that we currently allow to poll only one topic at once (multi-topic is planned for later). Then launch the `readstreams` service:
