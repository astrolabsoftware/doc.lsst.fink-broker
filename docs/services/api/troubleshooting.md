...
- **500 Internal Server Error:** The server encountered an unexpected condition that prevented it from fulfilling the request. Fink web service had typically an internal error. Let us know if that persists.
- **503 Service Unavailable:** The server is currently unable to handle the request due to temporary overload or maintenance. Fink components are probably in a bad shape.
- **504 Gateway Timeout:** Your query was too long to finish, and we cut it.